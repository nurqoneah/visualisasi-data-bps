...
def get_min(unit, df, year, title):
    ...
    if not filtered_df.empty:
        grouped_df = filtered_df.groupby('variable')[year].sum().reset_index().sort_values(year, ascending=True)
    else:
...
def get_max(unit, df, year, title):
    filtered_df = get_filter_df(df)
    if not filtered_df.empty:
        grouped_df = filtered_df.groupby('variable')[year].sum().reset_index().sort_values(year, ascending=False)
    else:
